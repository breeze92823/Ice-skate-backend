...
import { RinkState, PlayerState } from "./schema/RinkState.js";
...
function sanitizeProgress(raw: unknown): Partial<PlayerDoc> | null {
  ...
  if (Array.isArray(src.ownedTargets)) {
    out.ownedTargets = src.ownedTargets
...
/**
 * Single global room every client joins via `client.joinOrCreate("rink")`.
 ...
 */
export class RinkRoom extends Room<{ state: RinkState }> {
  state = new RinkState();

  // sessionId -> Bloxity user id, for whichever connected clients are signed
  // in. Deliberately NOT part of RinkState: unlike username/avatar this has
  ...
  messages = {
    ...
    // (speed/rebirth/wins/owned+equipped hex pads/auras/targets/moveSpeed) --
    ...
    saveProgress: async (client: Client, msg: unknown) => {
      ...
      try {
        ...
        console.warn("[RinkRoom] saveProgress failed", err);
      }
    },
  ...
  // this client, so the client can hydrate the fields RinkState doesn't
  // carry (owned/equipped hex pads, auras, targets, moveSpeed -- nobody else
  ...
  private async loadProgress(client: Client, userId: string, p: PlayerState) {
    ...
    try {
      ...
      client.send("progress", {
        ...
        ownedTargets: doc.ownedTargets ?? [],
        moveSpeed: doc.moveSpeed,
        moveSpeedLevelBonus: doc.moveSpeedLevelBonus ?? 0,
      });
    } catch (err) {
      console.warn("[RinkRoom] loadProgress failed", err);
    }
  }
    ...
    for (const stat of LEADERBOARD_STATS) {
      ...
      if (players) {
        try {
          ...
          console.warn(`[RinkRoom] leaderboard query failed for stat=${stat}`, err);
        }
      }
